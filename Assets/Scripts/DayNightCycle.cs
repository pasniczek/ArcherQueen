...
public class DayNightCycle : MonoBehaviour
{
    ...
    public void ControlPPV() // used to adjust the post processing slider.
    {
        ...
        {
            ppvNight.weight =  (float)mins / 60;
            ppvDay.weight = 1 - (float)mins / 60; // since dusk is 1 hr, we just divide the mins by 60 which will slowly increase from 0 - 1 
        ...
        {
            ppvNight.weight = 1 - (float)mins / 60; 
            ppvDay.weight =  (float)mins / 60;// we minus 1 because we want it to go from 1 - 0
            for (int i = 0; i < stars.Length; i++)
            {
